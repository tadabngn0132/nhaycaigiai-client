...
export function ProgramChooserModal({ open, onClose }: ProgramChooserModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-4xl rounded-lg bg-[#efede7] p-4 text-[#181817] shadow-[0_28px_90px_rgba(0,0,0,.55)] sm:p-5"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="mb-4 flex items-start justify-between gap-3 border-b border-[#d7d2c8] pb-3">
          <div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {registrationPrograms.map((program) => (
            ...
            >
              ...
              <span className="mt-3 text-xs font-extrabold uppercase text-[#111110] group-hover:text-[#c92c35]">
                Xem chi tiết
              </span>
            </Link>
          ))}
